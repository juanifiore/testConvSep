...
class FlexibleCNN(nn.Module):
    def __init__(self, num_layers=3, kernel_sizes=None, strides=None, input_channels=1):
        super().__init__()
...
configs = [
    {'num_layers': 2, 'kernel_sizes': [5, 5], 'strides': [1, 1]},
    {'num_layers': 3, 'kernel_sizes': [5, 5, 5], 'strides': [1, 1, 1]},
    {'num_layers': 4, 'kernel_sizes': [5, 5, 5, 5], 'strides': [1, 1, 1, 1]},
    {'num_layers': 5, 'kernel_sizes': [5, 5, 5, 5, 5], 'strides': [1, 1, 1, 1, 1]},
    {'num_layers': 6, 'kernel_sizes': [5, 5, 5, 5, 5, 5], 'strides': [1, 1, 1, 1, 1, 1]},
    {'num_layers': 7, 'kernel_sizes': [5, 5, 5, 5, 5, 5, 5], 'strides': [1, 1, 1, 1, 1, 1, 1]},
]
...
for i, config in enumerate(configs):
    ...
    np.save(f'../hendrix/grid/time_config{i+1}_layers{config["num_layers"]}.npy', np.array([duration]))

    sf.write(f'../hendrix/grid/vocals_config{i+1}_layers{config["num_layers"]}.wav', y, sr)
    #loses_todas.append(loses)
    np.save(f'../hendrix/grid/loss_config{i+1}_layers{config["num_layers"]}.npy', loses)
    np.save(f'../hendrix/grid/mask_pred_config{i+1}.npy', pred)
