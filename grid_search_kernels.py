...
configs = [
    {'num_layers': 4, 'kernel_sizes': [3, 3, 3, 3], 'strides': [1, 1, 1, 1], 'kernel': 3},
    {'num_layers': 4, 'kernel_sizes': [5, 5, 5, 5], 'strides': [1, 1, 1, 1], 'kernel': 5},
    {'num_layers': 4, 'kernel_sizes': [7, 7, 7, 7], 'strides': [1, 1, 1, 1], 'kernel': 7},
    {'num_layers': 4, 'kernel_sizes': [9, 9, 9, 9], 'strides': [1, 1, 1, 1], 'kernel': 9},
]
...
for i, config in enumerate(configs):
    ...
    # predecir y reconstruir sobre los mismos chunkos
    with torch.no_grad():
        model.eval()
    ...
    np.save(f'../hendrix/grid/time_config{i+1}_kernel{config["kernel"]}.npy', np.array([duration]))

    sf.write(f'../hendrix/grid/vocals_config{i+1}_kernel{config["kernel"]}.wav', y, sr)
    #loses_todas.append(loses)
    np.save(f'../hendrix/grid/loss_config{i+1}_kernel{config["kernel"]}.npy', loses)
    np.save(f'../hendrix/grid/mask_pred_config{i+1}_kernel{config["kernel"]}.npy', pred)
    del model
